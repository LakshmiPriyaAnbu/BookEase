  ...
  template: `
    ...
    <div style="padding:64px 40px;background:#FBFBFC;min-height:calc(100vh - 66px);display:flex;flex-direction:column;align-items:center;">
      ...
      <ng-container *ngIf="booking; else loading">
        ...
        <h2 style="font-family:'Space Grotesk',sans-serif;font-weight:700;font-size:32px;letter-spacing:-0.02em;margin:0 0 8px;color:#0F1115;">Booking confirmed!</h2>
        <p style="font-size:16px;color:#545A66;margin:0 0 32px;text-align:center;">
          We've sent the details and a calendar invite to
          <span style="color:#16181D;font-weight:600;">{{ booking.fullName ?? 'your email' }}</span>
        </p>
        ...
        <div style="width:100%;max-width:460px;background:#fff;border:1px solid #ECECEF;border-radius:18px;padding:26px 28px;margin-bottom:28px;">
          <div style="display:flex;justify-content:space-between;align-items:center;padding-bottom:16px;border-bottom:1px dashed #E2E3E9;margin-bottom:16px;">
            <span style="font-size:13px;color:#8A909C;font-weight:600;">Booking ID</span>
        ...
        <div style="display:flex;gap:14px;">
          <a routerLink="/" style="text-decoration:none;background:#fff;border:1px solid #DDDFE6;color:#16181D;font-weight:700;font-size:14.5px;padding:13px 26px;border-radius:12px;">Back to Home</a>
          <a routerLink="/admin/bookings" style="text-decoration:none;background:#5B4BE3;color:#fff;font-weight:700;font-size:14.5px;padding:13px 26px;border-radius:12px;box-shadow:0 8px 20px rgba(91,75,227,0.3);">View Bookings</a>
        </div>
      </ng-container>

      <ng-template #loading>
        <p style="color:#8A909C;font-size:15px;margin-top:80px;">Loading booking details…</p>
      </ng-template>
    </div>
  `,
  styles: [`
